...
$(document).ready(function() {
    ...
    $('.addNewQuestion').on('click', function(e) {
        ...
        var questionString = `<div class="drop-box">
                
        </div>.
        <div id="` + getIndex + `" class="input-container"  draggable="false">
    ...
    // Clear btn
    $(".container").on('click', '.clear-btn', function(e) {
        var blockid = $(this).parent().closest('.input-container').attr('id');
        // var preType = $('#' + blockid + ' .client-input-field input:first-child').attr('type');
        if ($('#' + blockid + ' .q_ans')) {
            $('#' + blockid + ' .q_ans').prop('selectedIndex', 0);
        }
        $('#' + blockid + ' .client-input-field input').each(function() {
            if ($(this).attr('type') == 'radio' || $(this).attr('type') == 'checkbox') {
                $(this).prop('checked', false);
            }
            // else if ($(this).attr('type') == 'select') {
            //     $('#' + blockid + ' .q_ans').prop('selectedIndex', -1);
            // }
            else {
                $(this).val('');
            }
